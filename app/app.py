...
def cria_senha(senha, name, role):
    global global_uid
    salt = bcrypt.gensalt()
    hashed = bcrypt.hashpw(senha.encode('utf-8'), salt)
    hashed = hashed.decode('utf-8')
    ## mudei aqui ##                                                                                                                                                                              
    cursor.execute(f"INSERT INTO sissal.users (userName, userPassword, userCard, userRole) VALUES ('{name}', '{hashed}', '{global_uid}', '{role}') ON DUPLICATE KEY UPDATE userPassword = '{hashed}', userCard = '{global_uid}'")
    connection.commit()                                                                                             
    return True
...
### Adicionei esse endpoint ###
@app.route('/ler', methods=['GET'])
def ler_cartao():
    global global_uid
    global_uid = request.args.get('uid')
    return jsonify({"status": "UID recebido", "uid": global_uid})
##################################################################
...
def Cadastro():
    ...
    role = data.get('idfuncoes')
    print("ESTOU AQUI", name, password)

    ### adicionei aqui ####
    if global_uid is None:
        resposta = {'mensagem': 'Por favor, leia o cartão antes de cadastrar.'}
    ...
    try:
        ...
        print(f"Não foi possível localizar o usuário. Erro: {e}")
        resposta = {'mensagem': 'Login Falhou!'}
        return jsonify(resposta)

### adicionei essa parte também ###
...
# Rota para lidar com a requisição do Arduino
...
def check_permission():
    try:
        ...
        # Obtém o UID do cartão da requisição
        card_uid = request.args.get('card_uid')
        room_number = request.args.get('sala')

        # Log dos valores recebidos
        print(f"Recebido card_uid: {card_uid}, sala: {room_number}")

        # Consulta no banco de dados
        with connection.cursor() as cursor:
            sql = """
                SELECT BookingRooms.userCard, BookingRooms.startHour, BookingRooms.endHour, BookingRooms.bookingDate 
                FROM BookingRooms 
                INNER JOIN Users ON BookingRooms.userId = Users.userId
                WHERE BookingRooms.roomNumber = %s
            """
        ...
        # Log da hora e data atuais
        print(f"Hora atual: {current_time}, Data atual: {current_date}")

        for result in results:
            userCard, startHour, endHour, bookingDate = result
            startHour_str = timedelta_to_str(startHour)
            endHour_str = timedelta_to_str(endHour)

            # Log de cada verificação
            print(f"Verificando userCard: {userCard}, startHour: {startHour_str}, endHour: {endHour_str}, bookingDate: {bookingDate}")
            if bookingDate.strftime("%Y-%m-%d") == current_date and startHour_str <= current_time <= endHour_str:
                if str(userCard) == card_uid:
                    print("Acesso Permitido")
                    return card_uid

        print("Acesso Negado")
        return "Acesso Negado ou o cartão não está cadastrado", 404
...
def Salas():
    ...
    future = executor.submit(verifica_aluno)
    resposta = future.result()
    print("Passei aqui", resposta)
    if resposta == 'Aluno':
        return render_template('PagInicial.html')
    else:
        ...
        with app.app_context():
            with db_lock:
                try:
                    ...
                    cursor.execute(f"SELECT * from sissal.bookingrooms WHERE startHour = '{horaConsulta}' AND bookingDate = '{dataConsulta}' AND roomNumber = {sala}")
                    result = cursor.fetchall()
                    ...
                    print(f"ID_user: {ID_user}, Card: {Card}, username: {username}, horaConsulta: {horaConsulta}, dataConsulta: {dataConsulta}, sala: {sala}")

                    if len(result) == 0:
                        cursor.execute(f"""
                            INSERT INTO sissal.bookingrooms 
                            (userId, roomId, roomNumber, userCard, userName, startHour, endHour, bookingDate)
                            VALUES ({ID_user}, {ID_room}, {sala}, '{Card}', '{username}', '{horaConsulta}', ADDTIME('{horaConsulta}', '02:00:00'), '{dataConsulta}')
                        """)
                        resposta = {'mensagem': 'True'}
                        connection.commit()
                        return resposta
